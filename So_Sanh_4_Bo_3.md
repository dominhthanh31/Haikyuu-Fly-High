# SO SÁNH 4 BỘ 3 — Bunkasai · Jazz · Fuyuyasumi · Kaisuiyoku
---

## I. ĐÁNH GIÁ TỔNG THỂ

| Bộ 3 | Setter | Tay đập | Thành viên 3 | Điểm |
|---|---|---|---|---|
| Bunkasai | Oikawa Tooru | Ushijima Wakatoshi | Iwaizumi Hajime | **9.5/10** |
| Jazz | Kageyama Tobio | Hinata Shoyo | Nishinoya Yuu ⚠️ | **9/10** |
| Fuyuyasumi | Miya Atsumu | Miya Osamu | Tendo Satori | **8/10** |
| Kaisuiyoku | Akaashi Keiji | Bokuto Kotaro | Aone Takanobu | **7.5/10** |

> ⚠️ = Thông tin từ leak

### Bunkasai — 9.5/10
- **Tấn công:** Oikawa 100% Nice Play mọi chạm bóng → Ushijima tự scale [Ý thức] vô hạn → sát thương liên tục. Iwaizumi hybrid đập × 360% (Kizuna) + đỡ bóng × 360%.
- **Phòng thủ:** Iwaizumi [Sức mạnh] đồng đội + 40% + hồi thể lực. Đỡ bóng → buff đồng đội tấn công (Build A).
- **Kizuna:** [Sức mạnh]/[Tinh thần] đồng đội + 24% (3 Bunkasai). Oikawa chuyền → Iwaizumi đập × 360% + [Phát Huy].
- **Điểm yếu:** Phòng thủ đội phụ thuộc chủ yếu vào Iwaizumi. Ushijima không buff đồng đội.

### Jazz — 9/10
- **Tấn công:** Hinata trần sát thương cao nhất (× 320% + 60% + 600% Uy lực). Kageyama [Chi Phối] Nice Play system + CH IV [Sức mạnh] + 100% cho tốc công không chắn.
- **Phòng thủ:** Nishinoya đỡ bóng + cứu bóng (× 400% PERFECT) + chuyền bóng hybrid.
- **Kizuna:** [Tốc công]/[Chuyền bóng] đồng đội + 5%/member Jazz. Tốc công synergy toàn diện.
- **Điểm yếu:** Hinata phụ thuộc đội (CH IV triệt [Ý thức]). Nishinoya vẫn LEAK → thiếu tiềm năng.

### Fuyuyasumi — 8/10
- **Tấn công:** Osamu hybrid tốc công/chuyền bóng, CH IV chuyền thành tuyệt chiêu từ đầu trận. Atsumu giao bóng kép (vô hiệu tuyệt chiêu đối thủ).
- **Phòng thủ:** Tendo chắn bóng [Ý thức] thay [Phản xạ], gamble ± 65%/85%. Atsumu buff [Kỹ thuật phòng thủ] đồng đội.
- **Kizuna:** [Sức mạnh] Atsumu/Osamu + 30%. [Kỹ thuật] + 2/2%/member. Tendo Nice Play đầu rally guaranteed.
- **Điểm yếu:** Tendo bất ổn (gamble). Osamu trần sát thương thấp. [Cơ Trí] yêu cầu [Ý thức] ≤ 30%.

### Kaisuiyoku — 7.5/10
- **Tấn công:** Bokuto [Đoàn Kết] cộng dồn → tuyệt chiêu bùng nổ mạnh khi [Phong Độ]. Akaashi đập hai × 260% không chắn.
- **Phòng thủ:** Akaashi buff [Đỡ bóng]/[Chắn bóng] đồng đội + 25% (trước bùng nổ). Aone chắn bóng + debuff chuyền đối thủ.
- **Kizuna:** Tinh thần đội + 20/rally. Chắn 2/3 người → [Phản xạ] + 10%.
- **Điểm yếu:** Bokuto bắt đầu [Thăng Trầm] → không tuyệt chiêu. Aone buff chính khi rời sân → mâu thuẫn. Akaashi buff phòng thủ mất khi bùng nổ.

---

## II. SO SÁNH 4 TAY ĐẬP

| Tiêu chí | Hinata Jazz | Bokuto Kaisuiyoku | Ushijima Bunkasai | Osamu Fuyuyasumi |
|---|---|---|---|---|
| **Kiểu tấn công** | Tốc công | Đập mạnh | Đập mạnh | Tốc công + Chuyền |
| **Trần sát thương** | 10 | 9 | 9.5 | 7 |
| **Tự lập** | 5 | 7 | 9.5 | 6 |
| **Buff đồng đội** | 6 | 9 | 4 | 9 |
| **Phòng thủ** | 7 | 6 | 3 | 4 |
| **Linh hoạt đội hình** | 5 | 7 | 8 | 9 |

### Hinata Jazz — Trần sát thương cao nhất
- Tuyệt chiêu × 320% + 60% threshold + [Không nóng vội] max + 600% Uy lực → cuối trận gần như one-shot.
- CH IV triệt [Ý thức] (- 100%/stack) → cần Nice Play từ nguồn ngoài (Kageyama, Oikawa).
- Phòng thủ bất ngờ tốt: Kỹ năng 3 chắn bóng + Kizuna buff tốc công/chuyền bóng.
- Phụ thuộc đội nặng → hẹp đội hình nhưng khi đủ điều kiện thì mạnh nhất.

### Bokuto Kaisuiyoku — Buff đội + bùng nổ
- [Đoàn Kết] buff chỉ số chính đồng đội + 0.16%/stack (max 199) + bùng nổ chỉ số chính toàn đội + 20%.
- Chu kỳ [Thăng Trầm]/[Phong Độ]: đầu trận yếu (không tuyệt chiêu), sau bùng nổ tinh thần cực mạnh.
- CH II: [Sức mạnh] + [Ý thức] × 85% (max + 100%). CH IV: [Sức mạnh] + 3%/stack (max + 150%) khi [Phong Độ].
- Tay đập buff đội mạnh nhất trong 4 → vừa ghi điểm vừa nâng toàn đội.

### Ushijima Bunkasai — Tự lập nhất
- [Ý thức] × 2 (Kỹ năng 3) + 8%/Nice Play (max 99 stacks) → scale gần như vô hạn.
- Threshold 200%: giảm hồi chiêu. 350%: + [Đập mạnh] × 40%. 400%: + [Sức mạnh] + 0.7%/% vượt (max + 70%).
- CH IV: [Ý thức] + 100% đầu trận + [Đập mạnh] + 6%/rally (max + 30%).
- Giao bóng × 140% + debuff [Đỡ bóng] đối thủ × 100%. Gần như không buff đồng đội.

### Osamu Fuyuyasumi — Linh hoạt nhất
- Hybrid: tốc công × 255% (+ 60% threshold, không chắn) + chuyền bóng trở thành tuyệt chiêu từ đầu nhờ CH IV.
- [Ý thức] = 0 vĩnh viễn → chuyển hóa thành Uy lực (+ [Tốc công] × 2%/% giảm, max + 100%).
- Chuyển [Kỹ thuật tấn công] × 60% cho đồng đội khi chuyền bóng (CH IV).
- Trần sát thương thấp nhất nhưng linh hoạt nhất: vừa tấn công vừa chuyền bóng, chơi ở mọi đội hình.

---

## III. SO SÁNH 4 CHUYỀN HAI

| Tiêu chí | Kageyama Jazz | Oikawa Bunkasai | Akaashi Kaisuiyoku | Atsumu Fuyuyasumi |
|---|---|---|---|---|
| **Chất lượng chuyền** | 9 | 10 | 6 | 8 |
| **Buff tấn công đồng đội** | 9 | 10 | 9 | 9 |
| **Giao bóng** | 8 | 10 | 2 | 9 |
| **Buff phòng thủ đội** | 3 | 6 | 9 | 8 |
| **Tự ghi điểm** | 4 | 8 | 8 | 5 |
| **Nice Play hỗ trợ** | 9 | 10 | 8 | 9 |
| **Linh hoạt đội hình** | 5 | 8 | 6 | 6 |

### Oikawa Bunkasai — 10/10
- CH II: 100% Nice Play MỌI chạm bóng → cơ chế mạnh nhất game. [Sức mạnh] + 20%.
- [Dẫn dắt]: [Ý thức] tổng đội + 100%. CH IV: chuyển chỉ số sang [Phát Huy] (max chỉ số chính + 1500, [Sức mạnh] + 100%, [Kỹ thuật tấn công] + 10%).
- Giao bóng × 310% (4 [Dẫn dắt]).
- Toàn diện nhất, không có điểm yếu rõ ràng.

### Akaashi Kaisuiyoku — 8/10
- Setter + attacker hybrid: đập hai × 260% không chắn. CH IV cuối trận + [Chuyền bóng] × 0.6%/cộng dồn tích lũy (max + 100%) → × 360%.
- CH II: [Đỡ bóng]/[Chắn bóng] đồng đội + 25% (trước bùng nổ). Tinh thần + 40/rally. [Sức mạnh] + 30%.
- Kỹ năng 2: khi bùng nổ → [Ý thức] + 70% → chuyển cho spiker × 50% (max + 60%).
- Phòng thủ đội mạnh nhất trong 4 setter. Tự ghi điểm mạnh nhờ đập hai.

### Atsumu Fuyuyasumi — 8/10
- [Cơ Trí] cho đồng đội [Ý thức] ≤ 30%: [Kỹ thuật tấn công/phòng thủ] + 10%. Sau tuyệt chiêu: + 20%/+ 15%.
- Giao bóng kép xen kẽ: nhảy lượn × 140% (vô hiệu tuyệt chiêu đập) + mạnh × 160% (vô hiệu tuyệt chiêu đỡ).
- CH IV: [Kỹ thuật tấn công] + 35%, [Kỹ thuật phòng thủ] + 25% cho đồng đội > 40%. Guaranteed Nice Play khi [Kỹ thuật tấn công] ≥ 60%.
- Hạn chế: [Cơ Trí] yêu cầu [Ý thức] ≤ 30% → giới hạn đội hình.

### Kageyama Jazz — 8/10
- [Chi Phối] system: đồng đội chuyền bóng → tích stack → tiêu hao = Nice Play bảo đảm + [Chuyền bóng] + 5% + [Tốc công] + 5% toàn đội (max 20%). CH II: 3 stack đầu trận.
- Chuyền bóng × 250% (Kỹ năng 4) + passive buff chuyền bóng đồng đội = [Chuyền bóng] × 40% (Kỹ năng 1).
- CH IV: đồng đội tốc công → [Sức mạnh] + 40%. Không bị chắn → + 60% (tổng + 100%). Cực synergy với Hinata Jazz (Kỹ năng 4 không bị chắn) và Kizuna Biến Nhân.
- Giao bóng × 160% Nice Play bảo đảm + sức đập tốc công đồng đội + [Tốc công] × 60%.
- Buff tấn công đồng đội cực mạnh cho đội tốc công. Không buff phòng thủ, không tự ghi điểm (trừ giao bóng).

---

## IV. SO SÁNH THÀNH VIÊN THỨ 3

| Tiêu chí | Nishinoya Jazz ⚠️ | Iwaizumi Bunkasai | Aone Kaisuiyoku | Tendo Fuyuyasumi |
|---|---|---|---|---|
| **Vai trò** | Libero | WS (hybrid) | MB chắn | MB chắn + tốc công |
| **Phòng thủ cá nhân** | 9 | 10 | 8 | 6 |
| **Buff đồng đội** | 9 | 10 | 5 | 7 |
| **Debuff đối thủ** | 4 | 3 | 9 | 2 |
| **Tự ghi điểm** | 2 | 8 | 4 | 6 |
| **Ổn định** | 9 | 9 | 8 | 4 |
| **Phối hợp bộ 3** | 9 | 10 | 6 | 8 |

### Iwaizumi Bunkasai — Toàn diện nhất
- Tuyệt chiêu đỡ × 360% + Memory × 70% = thực chất × 610%. Kỹ năng 2 reset hồi chiêu → spam tuyệt chiêu.
- [Sức mạnh] đồng đội + 40% (Kỹ năng 2 + 20% + CH II + 20%) + hồi thể lực + 5. Build A đỡ Perfect → buff đồng đội đập × 30%.
- [Ý thức] + 20%/kỹ năng (max + 200%) → vượt 100% → [Tinh thần] + 80% (CH IV) + [Đỡ bóng] + 20% (Kỹ năng 1).
- Kizuna: Oikawa chuyền → Iwaizumi đập × 360% + [Phát Huy]. [Sức mạnh]/[Tinh thần] đội + 24%.
- Hybrid đỡ + đập → không mất slot tấn công, hỗ trợ đội lẫn tự ghi điểm.

### Nishinoya Jazz — Libero lý tưởng (LEAK)
- [Vững Chắc] stacking (+ 500 [Đỡ bóng] + × 30%/stack). Kỹ năng 3: + [Đỡ bóng] × 150%, không BAD → đỡ mọi cú đập.
- CH II: kỹ năng mới 〈Cứu Bóng Thần Tốc〉 = [Cứu bóng] × 400% PERFECT + tinh thần + 40.
- CH IV: khi chuyền bóng → [Sức mạnh] spiker × 30% (max + 100%) + [Kỹ thuật tấn công] × 30% (max + 40%).
- Chuyền bóng hybrid: [Chuyền bóng] × 150% + 30%/đồng đội chuyền → 3 đồng đội = tuyệt chiêu.
- Không tự ghi điểm (libero), nhưng phòng thủ → chuyền → buff spiker cực mạnh.

### Aone Kaisuiyoku — Debuff đối thủ mạnh nhất
- [Khóa Mục Tiêu] tracking đối thủ → [Chắn Bóng Kiên Cường] stacking: [Phản xạ] + 10%/stack (max + 200%).
- Giao bóng × 140% + hồi chiêu chuyền đối thủ + 4 → phá nhịp chuyền hai đối phương.
- Thresholds: > 2 (sức mạnh chắn + 10%), > 4 (hồi chiêu giảm), > 6 ([Tinh thần] + 10%), > 8 (hồi chiêu giảm nữa).
- CH IV: không ở sân → [Chắn bóng] hàng trước + 25% + [Kỹ thuật] đồng đội + 5~15%.
- Buff đội chủ yếu khi **rời sân** → mâu thuẫn vai trò chắn bóng chính trên sân.

### Tendo Fuyuyasumi — Gamble cao, trần cao nhưng sàn thấp
- [Ý thức] thay [Phản xạ] cho MỌI Nice Play → Nice Play: + 65%; không Nice Play: - 85%. Cực bất ổn.
- Tuyệt chiêu chắn × 240% + [Chắn bóng] × 20%/lần kích hoạt (scale vô hạn, đổi hồi chiêu tăng).
- CH II: [Ý thức] + 25%. [Tinh thần] + [Sức mạnh] × 60% (max + 60%).
- [Hiệp Lực]: đồng đội tuyệt chiêu 2 lần → [Kỹ thuật tấn công/phòng thủ] + 15%. CH IV: chỉ số chính đội + 1%/ult (max + 20%).
- Kizuna: mỗi [Nghỉ đông] + 2/2% [Kỹ thuật]. Tendo Nice Play đầu mỗi rally guaranteed.
- Ceiling rất cao khi chuỗi Nice Play, floor rất thấp khi miss → phụ thuộc may mắn và [Ý thức] nền.

---
